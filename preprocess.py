...
tif_path_train, tags_list, labels_map = data_helper.get_file_tag_list(train_csv_file, train_tif_dir)
jpg_path_train, _, _ = data_helper.get_file_tag_list(train_csv_file, train_jpeg_dir, ext='jpg')

print("Get train JPEG mean std")
jpg_train_mean_std = data_helper.get_dict_means_stds(jpg_path_train)

# Preprocessing training data
f = h5py.File(h5_train_file, "w")
N = len(tif_path_train)
...
for i in range(1,len(checkpnt)):
    ...
    filespath = tif_path_train[checkpnt[i-1]:checkpnt[i]]
    ...
    with ThreadPoolExecutor(cpu_count()) as pool:
        for img_array, targets in tqdm(pool.map(data_helper._train_transform_to_matrices,
                                                [(file_path, tag, labels_map, img_resize, jpg_train_mean_std)
...
jpg_test_mean_std = data_helper.get_dict_means_stds(jpg_path_test)

# Preprocessing testing data
f = h5py.File(h5_test_file, "w")
files_name = os.listdir(test_tif_dir)
N = len(files_name)
...
for i in range(1,len(checkpnt)):
    ...
    with ThreadPoolExecutor(cpu_count()) as pool:
        for img_array, file_name in tqdm(pool.map(data_helper._test_transform_to_matrices,
                                                  [(test_tif_dir, file_name, img_resize, jpg_test_mean_std)
