...
with h5py.File(config.h5_train_file, "r") as f:
...
x_train = HDF5Matrix(config.h5_train_file, "x_train")
y_train = HDF5Matrix(config.h5_train_file, "y_train")

idx = np.array(y_train)[:,0]
dataset = x_train[np.where(idx)[0]]

print(dataset.shape)
print(dataset[0,0:3,0:3,0])

batch_size = config.batch_size

# building model and loading weight(if exists)
if not os.path.isdir(config.resource_dir):
    os.mkdir(config.resource_dir)
...
for ep in range(1, epochs+1):
    ...
    info('Epoch %4d/%4d' % (ep, epochs))
    print('%26s  %9s | %7s | %7s | %7s | %7s | %7s |  %7s ' % 
              ('', 'Batch No.', 'MGlobal', 'D Loss', 
              'G Loss', 'X Loss', 'G(Zd) Loss', 'K'))
...
#            print(UP + UP)
        ...
        info("%(batch_index)4s/%(batch_len)4s | %(m_global).5f | " \
             "%(loss_discriminator).5f | %(loss_generator).5f | %(loss_real_x).5f | " \
             "%(loss_gen_x)10.5f | %(k).6f" % log_info)
